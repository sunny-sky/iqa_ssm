...
/*function getResolvedEvent(){
	$.ajax({
		type:"POST",
		url:base+"/getResolvedEvent.html",
		data:{
		},
		dataType:"json",
		success:function(data){
			if(data.value=="0"){
				...
				if(eventResolvedList==""){
					...
					var eventResolvedHtml = '<div id="tab-31" class="tab-pane active"><div class="ibox-title"><div class="ibox-tools"></div></div><div class="ibox-content"><table class="table table-striped table-bordered table-hover dataTables-example" > <thead> <tr><th style="text-align: center;">序号</th><th style="text-align: center;">问题名称</th><th style="text-align: center;">提问用户</th><th style="text-align: center;">问题时间</th><th style="text-align: center;">操作</th></tr></thead><tbody > ';
					...
					for (var i = 0; i < eventResolvedList.length; i++) {
						...
						+'<a class="questioninfo" href=base+"/showResolvedEvent.html?q='+eventResolvedList[i].uSERQUESTIONID+'">查看详情</a>'
						+'</td>'
						+'</tr>'
		
//						eventResolvedHtml.innerHTML = eventResolvedHtml;    
					}
					document.getElementById('option1').innerHTML= eventResolvedHtml+'</tbody></table></div></div>'
					$.parser.parse();
				}
			}
		}
		...
		success:function(data){
			if(data.value=="0"){
				...
				if(eventUnresolvedList==""){
					...
					var eventUnresolvedHtml = '<div id="tab-31" class="tab-pane active"><div class="ibox-title"><div class="ibox-tools"></div> </div><div class="ibox-content"> <table class="table table-striped table-bordered table-hover dataTables-example" > <thead> '
						+'<tr>'
					...
					$("#option1")[0].innerHTML = eventUnresolvedHtml+'</tbody></table></div></div> ';
